...
describe('ConfigManager', () => {
  ...
  describe('getConfigPath', () => {
    it('returns null when no config file exists', () => {
      const result = configManager.getConfigPath(tmpDir);
    ...
    it('finds .devcontainer/devcontainer.json', () => {
      ...
      const result = configManager.getConfigPath(tmpDir);
      expect(result).toBe(path.join(devcontainerDir, 'devcontainer.json'));
    });
    ...
    it('finds .devcontainer.json at root', () => {
      ...
      const result = configManager.getConfigPath(tmpDir);
      expect(result).toBe(path.join(tmpDir, '.devcontainer.json'));
    });

    it('prefers .devcontainer/devcontainer.json over .devcontainer.json', () => {
      const devcontainerDir = path.join(tmpDir, '.devcontainer');
      fs.mkdirSync(devcontainerDir);
      fs.writeFileSync(path.join(devcontainerDir, 'devcontainer.json'), '{"name":"folder"}');
      fs.writeFileSync(path.join(tmpDir, '.devcontainer.json'), '{"name":"root"}');
  ...
  describe('readConfig', () => {
    it('returns null config when no file exists', () => {
      const result = configManager.readConfig(tmpDir);
      expect(result.config).toBeNull();
      expect(result.configPath).toBeNull();
      expect(result.parseErrors).toHaveLength(0);
    });

    it('parses a valid devcontainer.json', () => {
      const devcontainerDir = path.join(tmpDir, '.devcontainer');
      fs.mkdirSync(devcontainerDir);
      ...
      fs.writeFileSync(path.join(devcontainerDir, 'devcontainer.json'), configContent);
      ...
      expect(result.configPath).toBe(path.join(devcontainerDir, 'devcontainer.json'));
      expect(result.parseErrors).toHaveLength(0);
    });
  ...
  "image": "node:18",
    ...
    it('reports parse errors with location info', () => {
      const devcontainerDir = path.join(tmpDir, '.devcontainer');
      fs.mkdirSync(devcontainerDir);
  ...
  "image": "node:18"
    ...
    it('recovers partial config from malformed JSONC', () => {
      const devcontainerDir = path.join(tmpDir, '.devcontainer');
      fs.mkdirSync(devcontainerDir);
