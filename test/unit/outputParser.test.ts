...
describe("formatEventForChannel", () => {
  it("formats text events as plain text with LF (no ANSI, no CR)", () => {
    const event: CliOutputEvent = {
      type: "text",
      level: 2,
      timestamp: 1700000000000,
    ...
    expect(formatEventForChannel(event)).toBe("Hello world\n");
  });

  it("passes raw build content through verbatim (with LF)", () => {
    const event: CliOutputEvent = {
      type: "raw",
      level: 2,
      timestamp: 1700000000000,
      text: "#5 ERROR: failed to solve: dockerfile parse error",
    };
    expect(formatEventForChannel(event)).toBe(
      "#5 ERROR: failed to solve: dockerfile parse error\n",
    );
  });

  it("marks start events", () => {
    const event: CliOutputEvent = {
      type: "start",
      level: 2,
      timestamp: 1700000000000,
    ...
    expect(formatEventForChannel(event)).toBe("▶ Building image\n");
  });

  it("marks stop events with elapsed seconds", () => {
    const event: CliOutputEvent = {
      type: "stop",
      level: 2,
      timestamp: 1700000003500,
    ...
    expect(formatEventForChannel(event)).toBe("✓ Building image (3.5s)\n");
  });

  it("marks progress events", () => {
    const event: CliOutputEvent = {
      type: "progress",
      level: 2,
      timestamp: 1700000000000,
    ...
    expect(formatEventForChannel(event)).toBe("… Downloading...\n");
  });
});
