...
async function createTestContainer(name: string): Promise<string> {
  const { stdout } = await execFileAsync('docker', [
    ...
    '--label', `devcontainer.local_folder=${FIXTURE_DIR}`,
    '--name', name,
    TEST_IMAGE,
    'sleep', '300',
  ]);
...
async function cleanupLeftovers(): Promise<void> {
  try {
    const { stdout } = await execFileAsync('docker', [
      'ps', '-aq', '--filter', `label=${TEST_LABEL}=true`,
    ]);
  ...
  } catch { /* Docker might not be running */ }
}
...
describeWithDocker('Docker Integration Tests', () => {
  ...
  describe('dockerExec - real container', () => {
    it('executes a command and returns stdout', async () => {
      ...
      const result = await dockerExec(containerId, ['echo', 'hello from container']);
    ...
    it('can write and read back files', async () => {
      ...
      const writeResult = await dockerExec(containerId, [
        'sh', '-c', 'echo "test content" > /tmp/testfile.txt',
      ]);
      expect(writeResult.exitCode).toBe(0);

      // Read it back
      const readResult = await dockerExec(containerId, ['cat', '/tmp/testfile.txt']);
  ...
  describe('ConfigManager - real filesystem', () => {
    it('detects devcontainer.json in the fixture directory', () => {
      const configManager = new ConfigManager();
      const configPath = configManager.getConfigPath(FIXTURE_DIR);

      expect(configPath).not.toBeNull();
      expect(configPath).toContain('devcontainer.json');
    });

    it('parses the fixture devcontainer.json correctly', () => {
      const configManager = new ConfigManager();
      const result = configManager.readConfig(FIXTURE_DIR);
    ...
    it('returns null for a directory with no config', () => {
      const configManager = new ConfigManager();
      const configPath = configManager.getConfigPath('/tmp');
